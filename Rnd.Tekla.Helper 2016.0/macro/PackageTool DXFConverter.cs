    ...
    public static class Script
    {
        ...
        public static void Run(Tekla.Technology.Akit.IScript akit)
        {
            ...
            try
            {
                ...
                SetDefFile(XS_IsImperial, XS_Fabricator);
            ...
            {
                System.Windows.Forms.MessageBox.Show("Problem running process\n" + e.Message + "\n" + e.StackTrace);
            }
        }

        private static void SetDefFile(string XS_IsImperial, string XS_Fabricator)
        {
            if (XS_IsImperial == "TRUE")
            {
                defFile = "tekla_dstv2dxf_imperial.def";
                string xs_fab = _GetFab(XS_Fabricator);

                var defFiles = ReadDefFiles();

                foreach (string file in defFiles)
                {
                    if (!String.IsNullOrEmpty(file.Trim()) && file.Contains("="))
                    {
                        var pair = file.Trim().Split('=');
                        string key = pair[0];
                        string def = pair[1];

                        if (xs_fab.ToLower().Contains(key.ToLower()))
                        {
                            defFile = def;
                            break;
                        }
        ...
        private static string[] ReadDefFiles()
        {
            string TS_Version = GetCurrentVersion();
            // packagetool
            var defCollection = System.IO.File.ReadAllLines(System.IO.Path.Combine(System.Environment.GetFolderPath(System.Environment.SpecialFolder.LocalApplicationData), "PackageTool", TS_Version, "DefCollection.ini"));
